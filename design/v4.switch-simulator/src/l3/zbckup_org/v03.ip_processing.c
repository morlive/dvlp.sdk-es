...
static status_t fragment_ipv4_packet(packet_buffer_t *packet, uint16_t mtu, port_id_t egress_port) {
    ...
    for (uint16_t i = 0; i < num_fragments; i++) {
        ...
        if (packet_append_data(&frag_packet, &frag_header, header_len) != STATUS_SUCCESS ||
            packet_append_data(&frag_packet, packet_data + data_offset + frag_offset, payload_size) != STATUS_SUCCESS) {
...
static status_t fragment_ipv6_packet(packet_buffer_t *packet, uint16_t mtu, port_id_t egress_port) {
    ...
    for (uint16_t i = 0; i < num_fragments; i++) {
        ...
        if (packet_append_data(&frag_packet, &frag_header, IPV6_HEADER_LEN) != STATUS_SUCCESS ||
            packet_append_data(&frag_packet, &frag_ext_header, fragment_header_size) != STATUS_SUCCESS ||
            packet_append_data(&frag_packet, packet_data + data_offset + frag_offset, payload_size) != STATUS_SUCCESS) {
            LOG_ERROR( LOG_CATEGORY_L3, "Failed to append data to fragment packet");
            packet_destroy(&frag_packet);
            free(packet_data);
