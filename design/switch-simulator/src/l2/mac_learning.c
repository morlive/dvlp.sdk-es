...
static inline void mac_learning_acquire_lock(void) {
    spinlock_acquire(&g_mac_learning.lock);
}
...
static inline void mac_learning_release_lock(void) {
    spinlock_release(&g_mac_learning.lock);
}
...
static void reset_port_learning_rate(port_id_t port_id, uint32_t current_time) {
    if (port_is_valid(port_id) && port_id < g_mac_learning.num_ports) {
...
static bool is_port_rate_limited(port_id_t port_id, uint32_t current_time) {
    ...
    if (rate->learned_count >= MAC_LEARNING_RATE_LIMIT) {
        if (!rate->rate_limited) {
            ...
            LOG_WARNING(LOG_CATEGORY_L2, "MAC learning rate limit reached on port %u", port_id);
...
status_t mac_learning_init(uint32_t num_ports) {
    LOG_INFO(LOG_CATEGORY_L2, "Initializing MAC learning");
    
    if (g_mac_learning.initialized) {
        LOG_WARNING(LOG_CATEGORY_L2, "MAC learning already initialized");
        return STATUS_ALREADY_EXISTS;
    }
    
    if (num_ports == 0) {
        LOG_ERROR(LOG_CATEGORY_L2, "Invalid number of ports: %u", num_ports);
    ...
    if (g_mac_learning.port_learning_enabled == NULL || g_mac_learning.port_rates == NULL) {
        LOG_ERROR(LOG_CATEGORY_L2, "Failed to allocate memory for MAC learning");
    ...
    LOG_INFO(LOG_CATEGORY_L2, "MAC learning initialized for %u ports", num_ports);
...
status_t mac_learning_cleanup(void) {
    LOG_INFO(LOG_CATEGORY_L2, "Cleaning up MAC learning");
    ...
    LOG_INFO(LOG_CATEGORY_L2, "MAC learning cleanup complete");
    return STATUS_SUCCESS;
}
...
status_t mac_learning_set_global_state(bool enable) {
    if (!g_mac_learning.initialized) {
        LOG_ERROR(LOG_CATEGORY_L2, "MAC learning not initialized");
    ...
    LOG_INFO(LOG_CATEGORY_L2, "MAC learning globally %s", enable ? "enabled" : "disabled");
    return STATUS_SUCCESS;
}
...
status_t mac_learning_set_port_state(port_id_t port_id, bool enable) {
    ...
    LOG_INFO(LOG_CATEGORY_L2, "MAC learning %s on port %u", enable ? "enabled" : "disabled", port_id);
    return STATUS_SUCCESS;
}
...
///    
///    if (packet == NULL) {
///        LOG_ERROR(LOG_CATEGORY_L2, "NULL packet pointer");
///        return STATUS_INVALID_PARAMETER;
///    }
///    
///    if (!port_is_valid(port_id) || port_id >= g_mac_learning.num_ports) {
...
///    // проблема:
///    // функция packet_get_ethernet_header ожидает 
///    // двойной указатель (ethernet_header_t **) в качестве второго аргумента, 
///    // но в вызове одинарный указатель (&eth_header).
///    //
///    // ethernet_header_t eth_header;
///    ethernet_header_t *eth_header = NULL; // Инициализируем указатель
///    status_t status = packet_get_ethernet_header(packet, &eth_header);
///    if (status != STATUS_SUCCESS) {
///        mac_learning_release_lock();
///        LOG_ERROR(LOG_CATEGORY_L2, "Failed to extract Ethernet header from packet");
///        return status;
///    }
///    
...
///        mac_learning_release_lock();
///        LOG_ERROR(LOG_CATEGORY_L2, "Failed to get VLAN ID from packet");
///        return status;
///    }
///    
///    // Don't learn from multicast/broadcast source MACs
///    if ((*eth_header).src_mac.addr[0] & 0x01) {
///        mac_learning_release_lock();
///        LOG_DEBUG(LOG_CATEGORY_L2, "Skipping learning for multicast/broadcast source MAC");
///        return STATUS_SUCCESS;
///    }
///    
...
///        mac_learning_release_lock();
///        LOG_DEBUG(LOG_CATEGORY_L2, "Skipping learning on port %u in non-forwarding STP state", port_id);
///        return STATUS_SUCCESS;
///    }
///    
///    // Check rate limiting
...
///    //status = mac_table_lookup((*eth_header).src_mac, vlan_id, &entry);
///    status = mac_table_lookup((*eth_header).src_mac, vlan_id, &port_id);
///   // // Затем, если вам нужен port_id из записи:
///    port_id_t existing_port = entry.port_id; // предполагая, что в структуре есть такое поле
///    
///
///    if (status == STATUS_SUCCESS) {
///        // MAC already exists
...
///            LOG_INFO(LOG_CATEGORY_L2, "MAC %02x:%02x:%02x:%02x:%02x:%02x moved from port %u to port %u on VLAN %u",
///                    (*eth_header).src_mac.addr[0], (*eth_header).src_mac.addr[1], (*eth_header).src_mac.addr[2],
///                    (*eth_header).src_mac.addr[3], (*eth_header).src_mac.addr[4], (*eth_header).src_mac.addr[5],
...
///      status = mac_table_add((*eth_header).src_mac, port_id, vlan_id, false);
///      if (status == STATUS_SUCCESS) {
///          LOG_DEBUG(LOG_CATEGORY_L2, "Learned new MAC %02x:%02x:%02x:%02x:%02x:%02x on port %u VLAN %u",
///                  (*eth_header).src_mac.addr[0], (*eth_header).src_mac.addr[1], (*eth_header).src_mac.addr[2],
///                  (*eth_header).src_mac.addr[3], (*eth_header).src_mac.addr[4], (*eth_header).src_mac.addr[5],
///                  port_id, vlan_id);
///
...
///          LOG_WARNING(LOG_CATEGORY_L2, "Failed to learn MAC: MAC table is full");
///      } else {
///          LOG_ERROR(LOG_CATEGORY_L2, "Failed to learn MAC: error %d", status);
///      }
///    }
///    else {
///      // Some other error during lookup
///      LOG_ERROR(LOG_CATEGORY_L2, "MAC table lookup failed with error %d", status);
///  }
///
///  mac_learning_release_lock();
///  return status;
...
status_t mac_learning_reset_stats(void) {
    ...
    LOG_INFO(LOG_CATEGORY_L2, "MAC learning statistics reset");
    return STATUS_SUCCESS;
}
...
status_t mac_learning_get_port_state(port_id_t port_id, bool *enabled) {
    ...
    if (enabled == NULL) {
        LOG_ERROR(LOG_CATEGORY_L2, "NULL enabled pointer");
        return STATUS_INVALID_PARAMETER;
    }
...
status_t mac_learning_process_aging(uint32_t current_time) {
    ...
    // Должно стать (с добавлением указателя на таблицу):
    //status_t status = mac_table_process_aging(&g_mac_table, current_time);
    //status_t status = mac_table_process_aging(mac_table_get_instance(), current_time);
    status_t status = mac_table_process_aging((mac_table_t*)mac_table_get_instance(), current_time);

    if (status != STATUS_SUCCESS) {
        LOG_ERROR(LOG_CATEGORY_L2, "MAC table aging process failed: %d", status);
    }
...
status_t mac_learning_flush(vlan_id_t vlan_id, port_id_t port_id) {
    ...
    if (port_id != PORT_ID_INVALID && (port_id >= g_mac_learning.num_ports || !port_is_valid(port_id))) {
        LOG_ERROR(LOG_CATEGORY_L2, "Invalid port ID: %u", port_id);
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (status != STATUS_SUCCESS) {
        LOG_ERROR(LOG_CATEGORY_L2, "Failed to flush MAC entries: %d", status);
        return status;
    }

    LOG_INFO(LOG_CATEGORY_L2, "Flushed dynamic MAC entries for VLAN %u, port %u",
...
status_t mac_learning_handle_port_state_change(port_id_t port_id, bool is_up) {
    ...
    if (!is_up) {
        LOG_INFO(LOG_CATEGORY_L2, "Port %u went down, flushing dynamic MAC entries", port_id);
...
status_t mac_learning_handle_vlan_state_change(vlan_id_t vlan_id, bool is_active) {
    ...
    if (!is_active) {
        LOG_INFO(LOG_CATEGORY_L2, "VLAN %u became inactive, flushing dynamic MAC entries", vlan_id);
