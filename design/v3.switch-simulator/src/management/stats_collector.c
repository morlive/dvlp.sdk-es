...
error_code_t stats_deinitialize (stats_context_t *ctx) {
    if (!ctx || !ctx->private_data) {
        return ERROR_INVALID_PARAMETER;
    }
...
status_t stats_cleanup(stats_context_t *ctx) {
    // Вызываем существующую функцию deinitialize
    error_code_t status = stats_deinitialize(ctx);  // Исправлено здесь

    // Преобразуем error_code_t в status_t
