...
status_t ip_create_packet(const void *src_addr, const void *dst_addr, uint8_t protocol,
    ...
    *packet = (packet_buffer_t *)malloc(sizeof(packet_buffer_t));
    ...
    //(*packet)->metadata.port = PORT_CPU;
    //(*packet)->metadata.port = PORT_TYPE_CPU;
    (*packet)->metadata.port = port_cpu_id();
    //(*packet)->metadata.vlan_id = VLAN_ID_DEFAULT;
...
///        LOG_DEBUG(LOG_CATEGORY_L3, "IPv6 next hop resolution not implemented");
///        return ERROR_NOT_IMPLEMENTED;
///    } else {
///        /* Resolve next hop MAC via ARP */
///        status = arp_resolve_next_hop(&route->next_hop.ipv4, &next_hop_mac);
///        if (status != STATUS_SUCCESS) {
///            LOG_DEBUG(LOG_CATEGORY_L3, "ARP resolution failed for next hop");
...
///    LOG_DEBUG(LOG_CATEGORY_L3, "Forwarding IP packet to port %d", route->egress_port);
...
static status_t forward_ip_packet(packet_buffer_t *packet, const route_entry_t *route) {
    ...
    if (route->is_ipv6) {
        ...
        /* Resolve next hop MAC via ARP */
        /* Используем gateway из IPv4 части union */
        const ipv4_addr_t *next_hop_ip = &route->route.ipv4.gateway;
//        const ipv4_addr_t *next_hop_ip = route_get_ipv4_gateway(route); // abstraction


        status = arp_resolve_next_hop(next_hop_ip, &next_hop_mac);
        if (status != STATUS_SUCCESS) {
            LOG_DEBUG(LOG_CATEGORY_L3, "ARP resolution failed for next hop %d.%d.%d.%d",
                     next_hop_ip->addr[0], next_hop_ip->addr[1], 
                     next_hop_ip->addr[2], next_hop_ip->addr[3]);
            /* Queue packet for ARP resolution */
            return ERROR_ARP_PENDING;
        }
    }

    // Ensure packet is large enough to contain Ethernet header
    if (packet->size < sizeof(ethernet_header_t)) {
        LOG_WARNING(LOG_CATEGORY_L3, "Packet too small to contain Ethernet header: %u bytes", packet->size);
    ...
    /* Send packet out the egress port */
    LOG_DEBUG(LOG_CATEGORY_L3, "Forwarding IP packet to port %d (interface %s)", 
             route->egress_port, route->interface_name);
    return port_transmit(route->egress_port, packet);
}
///* Альтернативно, если хотите использовать прямое обращение к next_hop union: */
///*
//static status_t forward_ip_packet(packet_buffer_t *packet, const route_entry_t *route) {
//    status_t status;
//    mac_addr_t next_hop_mac;
...
//        return ERROR_NOT_IMPLEMENTED;
//    } else {
//        // Если добавили next_hop union в структуру
//        status = arp_resolve_next_hop(&route->next_hop.ipv4, &next_hop_mac);
//        if (status != STATUS_SUCCESS) {
//            LOG_DEBUG(LOG_CATEGORY_L3, "ARP resolution failed for next hop");
//            return ERROR_ARP_PENDING;
