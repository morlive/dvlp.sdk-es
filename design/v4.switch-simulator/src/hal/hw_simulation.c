...
static bool hw_sim_port_is_valid(port_id_t port_id);  /* Добавить сюда */
...
status_t hw_sim_get_port_config(port_id_t port_id, port_config_t *config)
{
    /* Validate parameters */
    if (config == NULL) {
        LOG_ERROR(LOG_CATEGORY_HAL, "Invalid NULL config pointer");
        return STATUS_INVALID_PARAMETER;
    }

    if (port_id >= MAX_PORTS || !hw_sim_port_is_valid(port_id)) {
        LOG_ERROR( LOG_CATEGORY_HAL, "Invalid port ID: %d", port_id);
        return STATUS_INVALID_PORT;
    }

    /* Get the configuration from simulated hardware state */
//    memcpy(config, &sim_port_configs[port_id], sizeof(port_config_t));
    memcpy(config, &g_sim_state.ports[port_id].info.config, sizeof(port_config_t));

    LOG_DEBUG(LOG_CATEGORY_HAL,  "Retrieved configuration for port %d", port_id);
    return STATUS_SUCCESS;
}
...
/**
 * @brief Check if a port is valid in the hardware simulation context
 * 
 * @param port_id ID of the port to validate
 * @return bool True if the port is valid in hardware simulation context, false otherwise
 */
static bool hw_sim_port_is_valid(port_id_t port_id)
{
    /* Basic range check */
    if (port_id >= MAX_PORTS || port_id < 0) {
        return false;
    }
    ...
    if (port_id >= g_sim_state.port_count || !g_sim_state.ports[port_id].initialized) {
        return false;
    }
    
    /* All checks passed, port is valid */
    return true;
}
