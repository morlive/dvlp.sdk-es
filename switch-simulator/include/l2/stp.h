...
typedef struct {
    ...
    uint32_t num_ports;          /**< Number of ports in the switch */
} stp_config_t;
...
status_t stp_get_port_state(port_id_t port_id, vlan_id_t vlan_id, stp_port_state_t *stp_state);

/**
 * @brief Set state for a specific VLAN on a port
 *
 * @param port_id Port ID
 * @param vlan_id VLAN ID
 * @param state New port state for this VLAN
 *
 * @return status_t Status of the operation
 */
status_t stp_set_port_vlan_state(port_id_t port_id, uint16_t vlan_id, stp_port_state_t state);
