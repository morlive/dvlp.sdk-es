 ...
 */
#ifndef SDK_ES_SWITCH_SIMULATOR_INCLUDE_L3_IP_H
#define SDK_ES_SWITCH_SIMULATOR_INCLUDE_L3_IP_H
...
// IP MTU constraints
#define MIN_MTU     68     /**< Minimum MTU for IPv4 as per RFC 791 */
#define MAX_MTU     9000   /**< Maximum MTU (Jumbo frames) */

/**
 * @brief Macro to provide a more concise name for version_class_flow field
 * that better aligns with RFC 2460 terminology
 */
#define version_tc_fl version_class_flow

#define payload_len payload_length
//#define id identification
...
typedef struct {
    ...
    union {
      uint16_t total_length;     /**< Total length */
      uint16_t total_len;        /**< Same as length, alternate name */
    };

    // uint16_t identification;   /**< Identification */
    union {
        uint16_t identification;   /**< Identification */
        uint16_t id;               /**< Same as identification, alternate name */
    };
 ...
 * @return status_t Status code (OK if successful)
 */
status_t ip_ipv4_to_str(ipv4_addr_t addr, char *buf, size_t buf_size);
 ...
 * @return status_t Status code (OK if successful)
 */
status_t ip_ipv6_to_str(ipv6_addr_t addr, char *buf, size_t buf_size);
 ...
 * @return status_t Status code (OK if successful)
 */
status_t ip_addr_to_str(ip_addr_t addr, char *buf, size_t buf_size);
 ...
 * @return status_t Status code (OK if successful)
 */
status_t ip_str_to_ipv4(const char *str, ipv4_addr_t *addr);
 ...
 * @return status_t Status code (OK if successful)
 */
status_t ip_str_to_ipv6(const char *str, ipv6_addr_t *addr);
 ...
 * @return status_t Status code (OK if successful)
 */
status_t ip_ipv4_fragment(const uint8_t *packet, size_t packet_size, uint16_t mtu,
 ...
 * @return status_t Status code (OK if successful)
 */
status_t ip_ipv4_reassemble(uint8_t **fragments, size_t *fragment_sizes, uint32_t fragment_count,
...
#endif /* SDK_ES_SWITCH_SIMULATOR_INCLUDE_L3_IP_H */
